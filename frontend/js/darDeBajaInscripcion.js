...
async function cargarInscripciones(clienteId) {
    ...
    try {
        const respuesta = await apiFetch(`${INSCRIPCIONES_URL}/cliente/${clienteId}`);
...
function mostrarInscripciones(inscripciones) {
    ...
    if (inscripciones.length === 0) {
        tablaInscripciones.innerHTML = `
            <tr>
                <td colspan="5">El usuario no tiene inscripciones.</td>
            </tr>
        `;
    ...
    inscripciones.forEach(inscripcion => {
        ...
        const estado = inscripcion.activa ? "Activa" : "Inactiva";
        ...
        fila.innerHTML = `
            ...
            <td>
                ${
                    inscripcion.activa
                        ? `<button class="btn-peligro btn-dar-de-baja" data-id="${inscripcion.id}">Dar de baja</button>`
                        : "-"
                }
            </td>
        `;
...
async function darDeBajaInscripcion(id) {
    ...
    const confirmar = confirm("¿Está seguro de que desea dar de baja esta inscripción?");
    ...
    try {
        const respuesta = await apiFetch(`${INSCRIPCIONES_URL}/${id}/cancelar`, {
            method: "PUT"
        });
...
document.addEventListener("click", function (event) {
    if (event.target.classList.contains("btn-dar-de-baja")) {
